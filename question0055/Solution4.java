 ...
 * 贪心策略如下：
 * 假设我们现在在索引i的位置。
 * （1）如果索引i的值为0，那么我们不可能再继续前进了，这种情况舍弃。
 * （2）如果索引i的值不为0，那么我们下一步可以走到索引i + k(1 <= k <= nums[i])。
 * 而在索引i + k我们又可以走到索引i + k + p(1 <= p <= nums[i + k])，我们选取索引i + k的原则是i + k + p取得最大值。
 *
 * 贪心策略的证明：
 * （1）如果此时i + k + p的最大值仍然小于n - 1，说明经过此步还未能抵达数组中最后一个元素。
 *
 * 对于索引i而言，假设索引i下一步的最优解为索引i + k。
 * 索引i + k的下一步所能到达的范围是(i + k + 1) ~ (i + k + nums[i + k])。
 *
 ...
 * 因为我们最终肯定是要跨过索引i + k，我们本可以一步到达索引i + k的位置，下一步就跨过索引i + k了，现在我们第一步到达索引i + j的位置，
 * 下一步还不能保证跨过索引i + k。因此索引i下一步的最优解一定只可能是i + k。
 *
