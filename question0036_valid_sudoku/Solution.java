...
public class Solution {
  ...
  public boolean isValidSudoku(char[][] board) {
    List<int[]> rowFlags = new ArrayList<>(), columnFlags = new ArrayList<>(),
        palaceFlags = new ArrayList<>();
    for (int i = 0; i < 9; i++) {
    ...
    for (int i = 0; i < 9; i++) {
      for (int j = 0; j < 9; j++) {
        ...
        if (rowFlags.get(i)[tmp - '0' - 1] == 1 || columnFlags.get(j)[tmp - '0' - 1] == 1 || palaceFlags.get((i / 3) * 3 + j / 3)[tmp - '0' - 1] == 1) {
          return false;
        }
